...
# In most data science applications, we do not observe the 
# population, but rather a sample. As with the average and 
# standard deviation, the sample correlation is the most 
# commonly used estimate of the population correlation.
# This implies that the correlation we compute and use as a 
# summary is a random variable

# A less fortunate geneticist can only afford to take a random 
# sample of 25 pairs. The sample correlation for this random 
# sample can be computed using this code.
...
# of this random variable. Here, we recreate R 1000 times, and 
# plot its histogram. 
...
# We see that the expected value is the population correlation, the 
# mean of these Rs is 0.5, and that it has a relatively high standard 
# error relative to its size, # SD 0.147. This is something to keep in 
# mind when interpreting # correlations. It is a random variable, and 
# it can have a # pretty large standard error.
